...
bool PlayerSM::play_file( uint32 call_id, const std::string & filename )
{
    ...
    case UNKNOWN:
    case PLAYING:
        dummy_log_warn( MODULENAME, "play_file: ignored in state %s", StrHelper::to_string( state_ ).c_str() );
        return false;
    ...
    default:
        dummy_log_error( MODULENAME, "play_file: invalid state %s", StrHelper::to_string( state_ ).c_str() );
        return false;
    }
    ...
    if( !b )
    {
        dummy_log_error( MODULENAME, "play_file: voip service failed" );
        return false;
    }
...
void PlayerSM::stop()
{
    ...
    case UNKNOWN:
    case IDLE:
        dummy_log_warn( MODULENAME, "stop: ignored in state %s", StrHelper::to_string( state_ ).c_str() );
        ASSERT( false );
        return;
    ...
    default:
        dummy_log_error( MODULENAME, "stop: invalid state %s", StrHelper::to_string( state_ ).c_str() );
        ASSERT( false );
        return;
    }
...
void PlayerSM::on_play_failed( uint32 call_id )
{
    ...
    case IDLE:
    case PLAYING:
        dummy_log_warn( MODULENAME, "on_play_failed: ignored in state %s", StrHelper::to_string( state_ ).c_str() );
        return;
    ...
    default:
        dummy_log_error( MODULENAME, "on_play_failed: invalid state %s", StrHelper::to_string( state_ ).c_str() );
        return;
    }
